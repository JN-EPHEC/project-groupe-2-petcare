...
import { updateMedicalHistory } from '../../services/firestoreService';
import { uploadMedicalDocument } from '../../services/imageUploadService';
import { useAuth } from '../../context/AuthContext';

interface EditMedicalHistoryScreenProps {
  navigation: any;
  route: any;
}
...
export const EditMedicalHistoryScreen: React.FC<EditMedicalHistoryScreenProps> = ({ navigation, route }) => {
  const { history, pet, onSave } = route.params;
  const { user } = useAuth();

  const [type, setType] = useState<'maladie' | 'allergie' | 'chirurgie' | 'autre'>(history.type);
  const [title, setTitle] = useState(history.title);
  const [description, setDescription] = useState(history.description);
  const [date, setDate] = useState(new Date(history.date));
  const [existingDocs, setExistingDocs] = useState<string[]>(history.documents || []);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Modifier l'antécédent</Text>
        <View style={{ width: 28 }} />
      </View>

      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        <View style={styles.section}>
          <CustomPicker value={type} onValueChange={(value) => setType(value as any)} options={HISTORY_TYPES} icon="document-text" />
        </View>

        <View style={styles.section}>
          <Input label="Titre *" value={title} onChangeText={setTitle} />
          <Input label="Description *" value={description} onChangeText={setDescription} multiline numberOfLines={4} maxLength={200} />
          ...
          <View style={styles.inputGroup}>
            ...
            {Platform.OS === 'web' ? (
              <View style={styles.dateInputContainer}>
                ...
                <input type="date" value={date.toISOString().split('T')[0]} onChange={(e) => setDate(new Date(e.target.value))} style={{ flex: 1, border: 'none', background: 'transparent', fontSize: '16px', fontWeight: '600', color: colors.navy, outline: 'none', padding: '8px 0' }} />
              </View>
            ) : (
        ...
        <View style={styles.section}>
          ...
          {existingDocs.map((doc, index) => (
            <View key={`existing-${index}`} style={styles.fileItem}>
              <Ionicons name="document" size={24} color="#4CAF50" />
              <Text style={styles.fileName} numberOfLines={1}>Document {index + 1}</Text>
              <TouchableOpacity onPress={() => handleRemoveExistingDoc(index)}>
          ...
          {selectedFiles.map((file, index) => (
            <View key={`new-${index}`} style={styles.fileItem}>
        ...
        <TouchableOpacity style={[styles.saveButton, isSuccess && styles.saveButtonSuccess, isSaving && styles.saveButtonDisabled]} onPress={handleSave} disabled={isSaving || isSuccess}>
          {isSuccess ? (
            <>
